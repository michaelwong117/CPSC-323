...
/** 
 * Went to Office Hours and talked with ULAs 
 * about how to print out
 * the stack for pushd and popd
 * this is the effort of much labor
**/
...
int handle_push_pop_child(int argc, char **argv)
{
    if (strcmp(argv[0], "pushd") == 0 && argc == 2)
    {
    ...
    else if (strcmp(argv[0], "popd") == 0 && argc == 1)
    {
        if (top == 0)
        {
...
int handle_push_pop_parent(int argc, char **argv)
{
...
int handle_built_in(const CMD *cmdList)
{
    // 1. make a child, try redirect / set vars, try chdir, push / pop, print

    // 2. replay chdir and push / pop; the child’s stack isn’t shared
    ...
    if (pid == 0)
    {
        ...
        exit(handle_push_pop_child(cmdList->argc, cmdList->argv));
    }
    else
    {
        ...
        return handle_push_pop_parent(cmdList->argc, cmdList->argv);
    }
}
...
int handle_simple(const CMD *cmdList, bool bg, bool subcmd)
{
    ...
    {
        return handle_built_in(cmdList);
    }
