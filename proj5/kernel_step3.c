...
// takes in the process id and the array of pagetables
void init_pagetable(pid_t pid, x86_64_pagetable** res_pagetables)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        int res = allocate_physical_page(pid, &res_pagetables[i]);
        if (res == -1)
        {
...
// }
    ...
    case INT_SYS_PAGE_ALLOC: {
        ...
        // int r = allocate_physical_page(current->p_pid, &current->p_pagetable);
        ...
        log_printf("NOT mapping virtual address %p to physical addreses %p\n", 
                    addr, (uintptr_t) current->p_pagetable);

        if (r >= 0) {
            // virtual_memory_map(current->p_pagetable, addr, (uintptr_t) current->p_pagetable,
            //                    PAGESIZE, PTE_P | PTE_W | PTE_U);
