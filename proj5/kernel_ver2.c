...
void process_setup(pid_t pid, int program_number) {
    ...
    x86_64_pagetable* page2;
    x86_64_pagetable* page3;
    x86_64_pagetable* page4;
    x86_64_pagetable* page5;
    
    init_pagetable(pid, page1, page2, page3, page4, page5);

    assert(res_pagetables != NULL);
    processes[pid].p_pagetable = page1;
...
void init_pagetable(pid_t pid, x86_64_pagetable* page1, x86_64_pagetable* page2,
    x86_64_pagetable* page3, x86_64_pagetable* page4, x86_64_pagetable* page5)
{
    memset(page1, 0, sizeof(page1));
    memset(page2, 0, sizeof(page2));
    memset(page3, 0, sizeof(page3));
    memset(page4, 0, sizeof(page4));
    memset(page5, 0, sizeof(page5));

    int res = allocate_physical_page(pid, page1);
    res = allocate_physical_page(pid, page2);
    res = allocate_physical_page(pid, page3);
    res = allocate_physical_page(pid, page4);
    res = allocate_physical_page(pid, page5);
    
    if (res == -1)
    {
        res_pagetables = NULL; return;
    }

    page1->entry[0] = page2 | PTE_P | PTE_W | PTE_U;
    page2->entry[0] = page3 | PTE_P | PTE_W | PTE_U;
    page3->entry[0] = page4 | PTE_P | PTE_W | PTE_U;
    page4->entry[0] = page2 | PTE_P | PTE_W | PTE_U;
    page4->entry[0] = page2 | PTE_P | PTE_W | PTE_U;
    

// l1pagetable->entry[0] = l2pagetable
    ...
    for(uintptr_t addr = 0; addr < PROC_START_ADDR; addr += PAGESIZE){
        ...
        virtual_memory_map(res_pagetable, addr, vmap.pa,  
        PAGESIZE, PTE_P | PTE_W | PTE_U);
...
int allocate_physical_page(int8_t owner, x86_64_pagetable * curr_pt)
{
...
int allocate_n_pages(int8_t owner, int n, x86_64_pagetable * res_pagetables)
{
    int count = 0;
    for (uintptr_t addr = 0; addr < MEMSIZE_PHYSICAL; addr += PAGESIZE)
    {
        ...
        {
            assign_physical_page(addr, owner);
            x86_64_pagetable* curr_page = &res_pagetables[count];
            curr_page = (x86_64_pagetable *) addr;
            // res_pagetables[count] = (x86_64_pagetable *) addr;
            count++;
        }
        if (count == n) return 0;
    }
...
void exception(x86_64_registers* reg) {
    ...
    case INT_SYS_PAGE_ALLOC: {
        ...
        // here addr is a virtual address
