...
void process_setup(pid_t pid, int program_number) {
    ...
    init_pagetable(pid, res_pagetables);
...
void init_pagetable(pid_t pid, x86_64_pagetable* res_pagetables)
{
    x86_64_pagetable* res_pagetable = &res_pagetables[0];
    memset(res_pagetable, 0, sizeof(res_pagetables));

    for (int i = 0; i < 5; i++)
    {
        x86_64_pagetable* curr = &res_pagetables[i];
        int res = allocate_physical_page(pid, curr);
        if (res == -1)
        {
    ...
    // int res = allocate_n_pages(pid, 5, res_pagetables);
