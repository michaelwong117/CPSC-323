...
void boot(void) {
    ...
    7d7e:	b9 00 10 00 00       	mov    $0x1000,%ecx
    7d83:	ba 00 10 00 00       	mov    $0x1000,%edx
    7d88:	be 01 00 00 00       	mov    $0x1,%esi
void boot(void) {
    7d8d:	53                   	push   %rbx
    boot_readseg((uintptr_t) ELFHDR, 1, PAGESIZE, PAGESIZE);
    7d8e:	bf 00 00 01 00       	mov    $0x10000,%edi
void boot(void) {
    7d93:	50                   	push   %rax
    boot_readseg((uintptr_t) ELFHDR, 1, PAGESIZE, PAGESIZE);
    7d94:	e8 0d ff ff ff       	call   7ca6 <boot_readseg>
    while (ELFHDR->e_magic != ELF_MAGIC) {
    7d99:	8b 04 25 00 00 01 00 	mov    0x10000,%eax
    7da0:	3d 7f 45 4c 46       	cmp    $0x464c457f,%eax
    7da5:	75 f9                	jne    7da0 <boot+0x23>
    elf_program* eph = ph + ELFHDR->e_phnum;
    7da7:	0f b7 2c 25 38 00 01 	movzwl 0x10038,%ebp
    7dae:	00 
    elf_program* ph = (elf_program*) ((uint8_t*) ELFHDR + ELFHDR->e_phoff);
    7daf:	48 8b 04 25 20 00 01 	mov    0x10020,%rax
    7db6:	00 
    elf_program* eph = ph + ELFHDR->e_phnum;
    7db7:	48 6b ed 38          	imul   $0x38,%rbp,%rbp
    elf_program* ph = (elf_program*) ((uint8_t*) ELFHDR + ELFHDR->e_phoff);
    7dbb:	48 8d 98 00 00 01 00 	lea    0x10000(%rax),%rbx
    elf_program* eph = ph + ELFHDR->e_phnum;
    7dc2:	48 01 dd             	add    %rbx,%rbp
    for (; ph < eph; ++ph) {
    7dc5:	48 39 eb             	cmp    %rbp,%rbx
    7dc8:	73 21                	jae    7deb <boot+0x6e>
        boot_readseg(ph->p_va, ph->p_offset / SECTORSIZE + 1,
    7dca:	48 8b 73 08          	mov    0x8(%rbx),%rsi
    7dce:	48 8b 4b 28          	mov    0x28(%rbx),%rcx
    for (; ph < eph; ++ph) {
    7dd2:	48 83 c3 38          	add    $0x38,%rbx
        boot_readseg(ph->p_va, ph->p_offset / SECTORSIZE + 1,
    7dd6:	48 8b 53 e8          	mov    -0x18(%rbx),%rdx
    7dda:	48 8b 7b d8          	mov    -0x28(%rbx),%rdi
    7dde:	48 c1 ee 09          	shr    $0x9,%rsi
    7de2:	ff c6                	inc    %esi
    7de4:	e8 bd fe ff ff       	call   7ca6 <boot_readseg>
    for (; ph < eph; ++ph) {
    7de9:	eb da                	jmp    7dc5 <boot+0x48>
    kernel_entry();
    7deb:	ff 14 25 18 00 01 00 	call   *0x10018
