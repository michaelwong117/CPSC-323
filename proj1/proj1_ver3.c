...
string_t *output; // global string for output
...
// if print is 1 we should print as we tick, otherwise save the values
void tick(string_t* str_in, char c, int* idx, int lvl, int print, string_t *saved)
{
    ...
    switch (state.state)
    {
        ...
        if (c == '\\')
        {
            ...
            // if you're at base level of recursion and encounter \ at EOF,
        ...
        if (c == '{')
        {
            ...
            else if (strcmp(state.macroNAME->data, "expandafter") == 0)
            {
                ...
                *idx = *idx + 1;
                ...
                {
                    tick(AFTER, AFTER->data[new_idx], &new_idx, lvl+1, 0, expandedAFTER);
                    new_idx++;
                }
                int before_idx = 0;
                while (before_idx < BEFORE->size)
                {
                    tick(BEFORE, BEFORE->data[before_idx], &before_idx, lvl+1, 1, NULL);
                    before_idx++;
                }
                int after_idx = 0;
                while (after_idx < expandedAFTER->size)
                {
                    printf("%c", expandedAFTER->data[after_idx]);
                    after_idx++;
                }
            ...
            else // found a custom macro case
            {
                ...
                if (macroMATCH != NULL)
                {
                    ...
                    {
                        tick(expandedVALUE, expandedVALUE->data[new_idx], &new_idx, lvl+1, 1, NULL);
                        new_idx++;
                    }
...
{ 
    ...
    {  
        tick(input, input->data[idx], &idx, recursion_lvl, 1, NULL);
        idx++;
    }
    /** 
