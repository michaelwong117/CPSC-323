...
TEST_FILE ?= p-test.c
TEST_DIR = ./tests/
LINUX_TEST_DIR = ./tests/linux-test


vpath %.c ${TEST_DIR}
vpath %.c ${LINUX_TEST_DIR}
...
KERNEL_OBJS = $(OBJDIR)/k-exception.o $(OBJDIR)/kernel.o \
	$(OBJDIR)/k-hardware.o $(OBJDIR)/vm.o $(OBJDIR)/k-loader.o $(OBJDIR)/lib.o $(OBJDIR)/k-vm.o
...
$(OBJDIR)/linux-malloc.o: malloc.c malloc.h process-linux.h $(BUILDSTAMPS)
	$(call run,gcc -c -DUSE_LINUX -nostdlib -g -o $@ $<,COMPILE-LINUX $<)

$(OBJDIR)/linux-p-%.o: p-%.c process-linux.h $(BUILDSTAMPS)
	$(call run,gcc -c -DUSE_LINUX -I . -nostdlib -g -o $@ $<,COMPILE-LINUX $<)
