...
// gets the next free physical page, assigns it, and returns its address
uintptr_t get_physical_page_addr(int8_t owner);

void unassign_page(uintptr_t addr, pid_t pid);

// frees a bad pagetable that couldn't find space for its five pages
// actually frees the five unlike garbage_collect
void free_bad_pagetable(pid_t pid);

// find a free process slot and set it to runnable
int find_free_process();

void garbage_collect(int process, x86_64_pagetable * child);

// copies the original pagetable into copy with fork semantics
int fork_copy(pid_t child_pid, x86_64_pagetable * child, x86_64_pagetable * parent);

// updates the refcount of a page
void update_refcount(uintptr_t addr);

// bool check_virtual_addr_for_page(uintptr_t addr);
